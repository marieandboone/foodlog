...
function loadLogForDate(date) {
  ...
  // Reset active and resting calories fields before loading data for the new day
  document.getElementById("resting-calories").value = "2328"; // Default value or 0
  document.getElementById("active-calories").value = "0";

  // Load calorie deficit first (before loading food log)
  ...
  get(ref(db, `foodLogs/${userId}/${date}`))
    .then((snapshot) => {
      if (snapshot.exists()) {
        ...
        // No food log for the date, reset totals
        totalCalories = 0;
        totalProtein = 0;
        updateTotals();
      }
    ...
    .then((snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        document.getElementById("resting-calories").value =
          restingCalories || "2328"; // Use stored value or default
        document.getElementById("active-calories").value =
          activeCalories || "0"; // Use stored value or 0 by default
        ...
        // Reset the values if no deficit data exists
